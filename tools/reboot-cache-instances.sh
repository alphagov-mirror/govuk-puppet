...
function show_help {
  echo "Usage: gds aws govuk-integration-poweruser ./reboot-cache-instance.sh -e integration ip-1-2-3-4.eu-west-1.compute.internal"
  exit 64
}
...
if [ -z "${govuk_env}" ]; then echo "[Error] You must provide an environment" && show_help && exit 1; fi
if [ -z "${instance_private_dns_name}" ]; then echo "[Error] You must provide an instance_private_dns_name like ip-1-2-3-4.eu-west-1.compute.internal" && show_help && exit 1; fi

echo "Going to reboot $instance_private_dns_name in $govuk_env"

instance_id=$(aws ec2 describe-instances --filters "Name=private-dns-name,Values=$instance_private_dns_name" | jq -r .Reservations[0].Instances[0].InstanceId)
...
sleep 30 # wait for requests to complete
